...
from Coreferee_resolver import CorefereeResolver
...
typesystem_filename = 'TypeSystemCoreference.xml'
...
lua_communication_script_filename = "duui_coreference.lua"
...
class DUUIResponse(BaseModel):
    ...
    begin_resolve: List[int]
    end_resolve: List[int]
    model_name: str
    model_version: str
    model_source: str
    model_lang: str
...
@lru_cache_with_size
def load_model(language, variant):
    model_i = CorefereeResolver(language, variant)
    return model_i
...
def post_process(request: DUUIRequest):
    ...
    try:
        ...
        with model_lock:
            ...
            begin_resolve = result["begin_resolve"]
            end_resolve = result["end_resolve"]

    except Exception as ex:
        logger.exception(ex)
    return DUUIResponse(meta=meta, modification_meta=modification_meta, begin=begin, end=end, begin_resolve=begin_resolve, end_resolve=end_resolve, model_name=settings.model_name,
                        model_version=model_version, model_source=model_source, model_lang=model_lang, model_variant=settings.model_variant)
