...
class Inferencer():
    def __init__(self, model_name, device, max_input_length, batch_size, cache_dir) -> None:
        ...
        if model_name == 'flan-t5-large':
            ...
            config = AutoConfig.from_pretrained(ckpt, num_labels=2, finetuning_task="text-classification",
                                                revision='main', token=None, cache_dir=cache_dir)
            config.problem_type = "single_label_classification"

            self.tokenizer = AutoTokenizer.from_pretrained(ckpt, use_fast=True, revision='main', token=None,
                                                           cache_dir=cache_dir)
            self.model = AutoModelForSequenceClassification.from_pretrained(
                ckpt, config=config, revision='main', token=None, ignore_mismatched_sizes=False,
        ...
        for mini_batch_input, batch_org_chunk in zip(batch_input, batch_org_chunks):
            ...
            with torch.no_grad():
                ...
                if self.model_name == 'flan-t5-large':
                    ...
                    decoder_input_ids = torch.zeros((mini_batch_input['input_ids'].size(0), 1), dtype=torch.long).to(
                        self.device)
                    outputs = self.model(input_ids=mini_batch_input['input_ids'],
