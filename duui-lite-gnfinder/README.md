...
```bash
docker run docker.texttechnologylab.org/duui-lite-gnfinder:latest
```

## Run with a specific port
```bash
docker run -p 1000:9714 docker.texttechnologylab.org/duui-lite-gnfinder:latest
```

## Run within DUUI
```java
composer.add(new DUUIDockerDriver.
    Component("docker.texttechnologylab.org/duui-lite-gnfinder:latest")
    .withScale(iWorkers)
    .withImageFetching());
```
