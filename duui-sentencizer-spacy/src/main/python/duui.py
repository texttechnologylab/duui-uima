...
# TODO better fix error "No component 'XXX' found in pipeline." in a way without hardcoding these?
SPACY_TRF_DISABLED_PIPES = {
    "en": {'tagger', 'lemmatizer', 'attribute_ruler', 'ner'},
    "de": {'tagger', 'morphologizer', 'lemmatizer', 'attribute_ruler'},
}
...
@lru_cache_with_size
def load_cache_spacy_model(lang, variant):
    logger.info("Loading spaCy for \"%s\" in variant \"%s\"...", lang, variant)

    tool, _, size = variant.partition("-")
    if size == "":
        size = tool

    if tool != "ruler":
        model_name = SPACY_MODELS.get(size, {}).get(lang, None)
        if model_name is None:
            raise Exception(f"Unknown spaCy variant \"{variant}\" for language \"{lang}\"")
        ...
        if tool == "parser":
            ...
            nlp.disable_pipes(*SPACY_TRF_DISABLED_PIPES.get(lang, {}))
    else:  # ruler
        nlp = spacy.blank(lang)
        nlp.add_pipe("sentencizer")

    logger.info("Loaded spaCy pipeline: \"%s\"", ", ".join(nlp.pipe_names))
    return nlp


def load_spacy_model(lang, variant):
    model_load_lock.acquire()

    err = None
    try:
        logger.info("Getting spaCy model \"%s\" in variant \"%s\"...", lang, variant)
        nlp = load_cache_spacy_model(lang, variant)
...
def post_process(request: TextImagerRequest) -> TextImagerResponse:
    ...
    try:
        ...
        meta = AnnotationMeta(
            ...
            version=settings.annotator_version,
            modelName=settings.variant,  # only named "pipeline" if using nlp.meta
            modelVersion=spacy_meta["version"],  # always 0.0.0
        ...
        modification_meta = DocumentModification(
            ...
            comment=f"{settings.annotator_name} ({settings.annotator_version}), spaCy ({spacy.__version__})"
        )
