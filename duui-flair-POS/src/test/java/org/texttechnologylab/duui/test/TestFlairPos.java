...
class TestSuccessPos {
    ...
    private static void checkResults(JCas jCas) {
        Collection<POS> annotations = JCasUtil.select(jCas, POS.class);
    ...
    @Test
    @Disabled
    void test_large_xmi() throws Exception {
        JCas jCas = JCasFactory.createJCas();
        try (FileInputStream fs = new FileInputStream("src/test/resources/1.xmi")) {
            CasIOUtils.load(fs, jCas.getCas());
        }
