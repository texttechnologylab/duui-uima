...
function deserialize(inputCas, inputStream)
    ...
    for i, pSentiment in ipairs(results["sentiments"]) do
        ...
        local sentiment = luajava.newInstance("org.hucompute.textimager.uima.type.GerVaderSentiment", inputCas)

        sentiment:setBegin(pSentiment["iBegin"])
        sentiment:setEnd(pSentiment["iEnd"])
        sentiment:setPos(pSentiment["pos"])
        sentiment:setNeu(pSentiment["neu"])
        sentiment:setNeg(pSentiment["neg"])
        sentiment:setSentiment(pSentiment["compound"])
