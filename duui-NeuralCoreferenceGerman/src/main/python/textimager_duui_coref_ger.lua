...
function serialize(inputCas, outputStream)
    ...
    while sents:hasNext() do
        ...
        -- local tokens = util:select(inputCas, Token):iterator()
        ...
        while tokens:hasNext() do
            ...
            if token_begin >= begin_sen and token_end <= end_sen then
...
--     Get Token results
    ...
    for i, link in ipairs(links) do
        if link["write"] then
            local cas_link = luajava.newInstance("org.texttechnologylab.annotation.semaf.meta.MetaLink", inputCas)
            b_f = link["begin_fig"]
            e_f = link["end_fig"]
            b_g = link["begin_gr"]
            e_g = link["end_gr"]
