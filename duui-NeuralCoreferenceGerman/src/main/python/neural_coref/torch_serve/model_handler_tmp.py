...
class CorefHandler(BaseHandler):
    def initialize(self, context):
        ...
        self.runner = Runner("tuba10_electra_uncased", device_id, skip_data_loading=True, log_to_file=False)
        self.model = self.runner.initialize_model()
        self.model.to(self.device)
        self.tensorizer = Tensorizer(self.runner.config)
        self.model.load_state_dict(torch.load("models/se10_electra_uncased.mar", map_location=self.device))
        self.model.eval()
