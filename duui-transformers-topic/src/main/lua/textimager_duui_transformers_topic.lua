    ...
    if results["modification_meta"] ~= nil and results["meta"] ~= nil and results["selections"] ~= nil then
        ...
        for i, selection in ipairs(results["selections"]) do
            ...
            for j, sentence in ipairs(selection["sentences"]) do
                for k, topic in ipairs(sentence["topics"]) do
                    local topic_anno = luajava.newInstance("org.hucompute.textimager.uima.type.category.CategoryCoveredTagged", inputCas)
                    topic_anno:setBegin(sentence["sentence"]["begin"])
                    topic_anno:setEnd(sentence["sentence"]["end"])
                    topic_anno:setValue(topic["label"])
                    topic_anno:setScore(topic["score"])
                    topic_anno:addToIndexes()

                    local meta_anno = luajava.newInstance("org.texttechnologylab.annotation.AnnotatorMetaData", inputCas)
                    meta_anno:setReference(topic_anno)
                    ...
                    meta_selection:setReference(topic_anno)
