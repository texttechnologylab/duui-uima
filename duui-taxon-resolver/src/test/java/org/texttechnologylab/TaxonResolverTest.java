...
public class TaxonResolverTest {
	...
	static final String TAXON_RESOLVER_IMAGE = "duui-taxon-resolver:1.0.1";
	static final String TAXON_RESOLVER_REMOTE_HOST = "http://localhost:12345";
	static final Strategy STRATEGY = Strategy.DOCKER;
	...
	public void testSingleAnnotationsSameView() throws Exception {
		...
		assertInstanceOf(
			TaxonResolution.class,
			recognized.getResolutions(0),
			"Expected resolution to be of type TaxonResolution"
		);
		...
		assertEquals(
			"Hymenoptera",
			resolution.getOrderName(),
			"Expected order name to be 'Hymenoptera'"
		);
		...
		assertEquals(
			"Podalonia hirsuta",
			resolution.getSpeciesName(),
			"Expected species name to be 'Podalonia hirsuta'"
		);
		assertEquals(1347914, resolution.getSpeciesId(), "Expected species ID to be 1347914");
		assertEquals(
			"Podalonia hirsuta (Scopoli, 1763)",
			resolution.getScientificName(),
			"Expected scientific name to be 'Podalonia hirsuta (Scopoli, 1763)'"
		);
	}
		...
		for (int i = 0; i < recognized.getResolutions().size(); i++) {
			...
			switch (resolution.getProvider()) {
				case "gbif" -> {
					...
					assertEquals(
						1347914,
						resolution.getSpeciesId(),
						"Expected species ID to be 1347914 for GBIF"
					);
				}
				case "ncbi" -> {
					hasNcbi = true;
					...
					assertEquals(
						"Calamagrostis",
						resolution.getParentName(),
						"Expected parent name to be 'Calamagrostis' for NCBI"
					);
					assertEquals(15376, resolution.getParentId(), "Expected parent ID to be 15376 for NCBI");
					assertEquals(
						"Calamagrostis arenaria",
						resolution.getScientificName(),
						"Expected scientific name to be 'Calamagrostis arenaria' for NCBI"
					);
				...
				case "taxref" -> {
					...
					assertEquals(
						"Plantae",
						resolution.getKingdomName(),
						"Expected kingdom name to be 'Plantae' for TAXREF"
					);
					assertEquals(
						"Equisetopsida",
						resolution.getClassName(),
						"Expected class name to be 'Equisetopsida' for TAXREF"
					);
					assertEquals(
						"Poales",
						resolution.getOrderName(),
						"Expected order name to be 'Poales' for TAXREF"
					);
					assertEquals(
						"Poaceae",
						resolution.getFamilyName(),
						"Expected family name to be 'Poaceae' for TAXREF"
					);
					assertEquals(
						"Pooideae",
						resolution.getSubfamilyName(),
						"Expected subfamily name to be 'Pooideae' for TAXREF"
					);
					assertEquals(
						"Poeae",
						resolution.getTribeName(),
						"Expected tribe name to be 'Poeae' for TAXREF"
					);
					assertEquals(
						"Ammophila arenaria",
						resolution.getSpeciesName(),
						"Expected species name to be 'Ammophila arenaria' for TAXREF"
					);
					assertEquals(
						"Ammophila arenaria",
						resolution.getScientificName(),
						"Expected scientific name to be 'Ammophila arenaria' for TAXREF"
					);
					assertEquals(
						"Oyat des sables, Ammophile des sables, Oyat, Chiendent marin, Roseau des sables, Gourbet",
						resolution.getVernacularName(),
						"Expected vernacular name to match for TAXREF"
					);
					assertEquals(
						"Ammophila arenaria (L.) Link, 1827",
						resolution.getAcceptedNameUsage(),
						"Expected accepted name usage to be 'Ammophila arenaria (L.) Link, 1827' for TAXREF"
					);
					assertEquals(
						"(L.) Link, 1827",
						resolution.getAuthorship(),
						"Expected authorship to be '(L.) Link, 1827' for TAXREF"
					);
					assertEquals("species", resolution.getRank(), "Expected rank to be 'species' for TAXREF");
					assertEquals(
						"https://taxref.mnhn.fr/taxref-web/taxa/82139",
						resolution.getReferences(),
						"Expeceted references to be 'https://taxref.mnhn.fr/taxref-web/taxa/82139' for TAXREF"
					);
				}
	...
	@Test
	public void testFailQuietlyOnMissingTaxon() throws Exception {
		addTaxonResolver(null);
		...
		comment1.setReference(taxon);
		comment1.setKey("linking");
		comment1.setValue("GBIF:123521"); // Apparently 9999999 exists
		comment1.addToIndexes();
		
		AnnotationComment comment2 = new AnnotationComment(jcas);
		comment2.setReference(taxon);
		comment2.setKey("linking");
		comment2.setValue("NCBI:9999999");
		comment2.addToIndexes();
		
		AnnotationComment comment3 = new AnnotationComment(jcas);
		comment3.setReference(taxon);
		comment3.setKey("linking");
		comment3.setValue("TAXREF:9999999");
		comment3.addToIndexes();
		
		composer.run(jcas);
		
		storeCas(jcas, "missing_taxon");
		
		Collection<RecognizedTaxon> taxa = JCasUtil.select(jcas, RecognizedTaxon.class);
		assertEquals(1, taxa.size(), "Expected exactly one taxon to be recognized");
		RecognizedTaxon recognized = taxa.iterator().next();
		assertTrue(
			recognized.getResolutions().isEmpty(),
			"Expected no resolutions for a non-existent taxon"
		);
	}
